...
void http_server(int argc, char **argv) {
    ...
    args_parser.add<string>("db_path", 'd', "level_db存储路径", true, "");

    // 入参检查
    args_parser.parse_check(argc, argv);
    // 提取入参
    int http_port = args_parser.get<int>("http_port");
    string db_path = args_parser.get<string>("db_path");
    // http启动服务
...
//  router.Use(GlobalHttpHandler::Authorization);
    ...
    TracerHttpHandler tracer_http_handler(level_db_wrapper, redisUtil);
    // tracer请求链接
