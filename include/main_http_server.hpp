...
void http_server(int argc, char **argv) {
  ...
  router.preprocessor = HttpHandler::preprocessor;
//  router.AllowCORS();
//  router.Use(HttpHandler::Authorization);
  ...
  router.GET("/get_config", [&](HttpRequest *req, HttpResponse *resp) {
    json return_res = {
        {"success", false},
        {"msg", ""},
        {"result", ""}
    ...
    return resp->String(return_res.dump());
  });
