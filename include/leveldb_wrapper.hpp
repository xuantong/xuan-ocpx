...
public:
    ...
    // 反序列化模板函数
    template<typename T>
    std::unique_ptr<T> ProtoBufGet(const std::string& key) {
        ...
        if (!message->ParseFromString(value_str)) {
            throw std::runtime_error("Failed to parse protobuf message for key: " + key + ". Invalid or corrupted data.");
        }
