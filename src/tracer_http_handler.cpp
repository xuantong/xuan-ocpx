...
int TracerHttpHandler::tracer(HttpRequest *req, HttpResponse *resp) {
    ...
    if (tracerReq.action() == tracer::req::ActionType::EXPOSURE_ACTION) {
        ...
        // 发送请求
        req->method = HTTP_GET;
        req->url = async_req_url;
        auto resp = requests::request(static_cast<requests::Request>(req));
...
int TracerHttpHandler::callback(HttpRequest *req, HttpResponse *resp) {
    ...
    std::unique_ptr<tracer::req::TracerReq> tracerReq = level_db_wrapper_.ProtoBufGet<tracer::req::TracerReq>(
            tracerCallbackReq.id());
